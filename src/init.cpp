...
void PrepareShutdown()
{
    ...
    util::ThreadRename("cashcash-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), cashcash_CONF_FILENAME));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), cashcash_PID_FILENAME));
#endif
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all cashcash specific functionality (Masternodes, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-mnconf=<file>", strprintf(_("Specify masternode configuration file (default: %s)"), cashcash_MASTERNODE_CONF_FILENAME));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2015-%i The cashcash Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
{
    util::ThreadRename("cashcash-loadblk");
...
/** Sanity checks
 *  Ensure that cashcash is running in a usable environment with all
...
#endif
    LogPrintf("cashcash version %s (%s)\n", version_string, CLIENT_DATE);
}
...
static bool LockDataDirectory(bool probeOnly)
{
    ...
    // Make sure only a single cashcash process is using the data directory.
    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        do {
            ...
            try {
                ...
                //cashcash specific: zerocoin and spork DB's
                ...
                // cashcash: load previous sessions sporks if we have them.
