...
#include "qt/cashcash/forms/ui_snackbar.h"
#include "qt/cashcash/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(cashcashGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &cashcashGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
