...
#include "qt/cashcash/settings/settingsfaqwidget.h"
#include "qt/cashcash/settings/forms/ui_settingsfaqwidget.h"
#include <QScrollBar>
#include <QMetaObject>
#include "qt/cashcash/qtutils.h"

SettingsFaqWidget::SettingsFaqWidget(cashcashGUI *parent) :
    QDialog(parent),
    ui(new Ui::SettingsFaqWidget)
{
    ...
    QString introContent = formatFAQContent(
        formatFAQParagraph(
            tr("cashcash is a form of digital online money using blockchain technology "
               "that can be easily transferred globally, instantly, and with near "
               "zero fees. cashcash incorporates market leading security & "
               "privacy and is also the first PoS (Proof of Stake) Cryptocurrency "
               "to implement Sapling(SHIELD), a zk-SNARKs based privacy protocol.")) +
        formatFAQParagraph(
            tr("cashcash utilizes a Proof of Stake (PoS) consensus system algorithm, "
               "allowing all owners of cashcash to participate in earning block rewards "
    ...
    QString unspendablePIVContent = formatFAQContent(
        formatFAQParagraph(
            tr("Newly received cashcash requires 6 confirmations on the network "
               "to become eligible for spending which can take ~6 minutes.")) +
        formatFAQParagraph(
            tr("Your cashcash wallet also needs to be completely synchronized "
    ...
    QString stakeContent = formatFAQContent(
        formatFAQOrderedList(
            ...
            formatFAQListItem(tr("You must have a balance of cashcash with a minimum of 600 confirmations.")) +
        ...
        formatFAQParagraph(
            tr("You can see the status of staking in the wallet by mousing over the "
               ...
               "will confirm that staking is active.")
                .arg("cashcash-core", "<span style=\"font-style:italic\">getstakingstatus</span>")));
    ...
    QString supportContent = formatFAQContent(
        formatFAQParagraph(
            tr("We have support channels in most of our official chat groups, for example %1")
                .arg("<a style='color: #b088ff' href='https://discord.cashcash.digital'>" + tr("#support in our Discord") + "</a>.")));
    ...
    QString masternodeContent = formatFAQContent(
        ...
        formatFAQParagraph(
            tr("Masternode Perks:") +
            formatFAQUnorderedList(
                formatFAQListItem(tr("Participate in cashcash Governance")) +
                formatFAQListItem(tr("Earn Masternode Rewards")) +
                formatFAQListItem(tr("Commodity option for future sale")) +
                formatFAQListItem(tr("Help secure the cashcash network")))) +
        formatFAQParagraph(
            tr("Requirements:") +
            formatFAQUnorderedList(
    ...
    if (parent)
        connect(parent, &cashcashGUI::windowResizeEvent, this, &SettingsFaqWidget::windowResizeEvent);
}
