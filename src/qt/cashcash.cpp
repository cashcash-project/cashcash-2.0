...
#include "config/cashcash-config.h"
#endif
...
#include "qt/cashcash/splash.h"
#include "qt/cashcash/welcomecontentwidget.h"
...
{
    return QCoreApplication::translate("cashcash-core", psz).toStdString();
}
...
static void initTranslations(QTranslator& qtTranslatorBase, QTranslator& qtTranslator, QTranslator& translatorBase, QTranslator& translator, bool forceLangFromSettings = false)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in cashcash.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in cashcash.qrc)
...
/** Main cashcash application object */
...
public:
    ...
    /// Get window identifier of QMainWindow (cashcashGUI)
...
{
    window = new cashcashGUI(networkStyle, 0);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, &cashcashGUI::detectShutdown);
}
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &cashcashGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (pwalletMain) {
            ...
            window->addWallet(cashcashGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(cashcashGUI::DEFAULT_WALLET);
        ...
        // cashcash: URIs or payment requests:
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &cashcashGUI::handlePaymentRequest);
        connect(window, &cashcashGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
{
    QMessageBox::critical(0, "Runaway exception", QObject::tr("A fatal error occurred. cashcash can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    Q_INIT_RESOURCE(cashcash_locale);
    Q_INIT_RESOURCE(cashcash);
    ...
    // cashcash: links repeatedly have their payment requests routed to this process:
