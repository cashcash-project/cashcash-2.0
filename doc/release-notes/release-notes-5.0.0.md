cashcash Core version *v5.0.0* is now available from:  <https://github.com/cashcash-project/cashcash/releases>
...
cashcash Core v5.0.0 is a mandatory update for all users. This release contains a protocol upgrade (to `70920`), new consensus rules and improvements that are not backwards compatible with older versions. Users will need to update their clients before enforcement of this update goes into effect.
...
For the other packages, the user must save the param files in the proper location, before being able to run cashcash v5.0.0:
...
More visual information about the protocol can be found at https://cashcash.digital .
...
See the release notes from [v4.2.0](https://github.com/cashcash-Project/cashcash/blob/master/doc/release-notes/release-notes-4.2.0.md#label-and-account-apis-for-wallet) for a full description of the changes from the 'account' API to the 'label' API.
...
| `getaccountaddress` | Returned the current cashcash address associated with an internal account | Account system removal |
| `setaccount` | Associate a cashcash address with an internal account name | Account system removal |
| `getaccount` | Returned the internal account associated with a given cashcash address | Account system removal |
| `getaddressesbyaccount` | Returned a list of cashcash addresses associated with an internal account | Account system removal |
...
| `sendfrom` | Send an amount from an internal account to a cashcash address | Account system removal |
...
| `multisend` | Configure sending a portion of stake rewards to a cashcash address | Pending a full rewrite |
...
### Build Systems
- #1856 `1bbb906059` [CMake] Add test_cashcash unit test target (Fuzzbawls)
...
- #2041 `a15dc3417d` [Build] CMake cashcashd and unit tests: fixing missing MacOS framework. (furszy)
