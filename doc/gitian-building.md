...
*Setup instructions for a gitian build of cashcash Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the cashcash
...
to the cashcash GitHub release page.
...
- [Building cashcash Core](#building-cashcash-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/cashcash-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/cashcash-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/cashcash-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/cashcash-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/cashcash-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/cashcash-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/cashcash-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@cashcash.digital) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/cashcash-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/cashcash-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/cashcash-osx-*-build.assert
```
